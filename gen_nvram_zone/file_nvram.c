...
#include "nvram/nvram_core.h"

unsigned char* AES_128_KEY = (unsigned char*)"xxxxxxxxxxxxxxxx";
...
/* Dynamic NVRAM size */
unsigned int NVRAM_MTD_SIZE = 0x20000;  /* Default 128K */
...
/* Globals */
char *nvram_values = NULL;
unsigned long nvram_offset = 0;
...
int nvram_set_temp(const char *name, const char *value, int is_temp)
{
	int ret;
	char *out = NULL;
...
int nvram_init(char *img_fname, int size)
{
	...
	if (ret != size) {
		printf("write file %s failed. errno %d", img_fname, errno);
		free(buf);
		close(fd);
		return -1;
	}
